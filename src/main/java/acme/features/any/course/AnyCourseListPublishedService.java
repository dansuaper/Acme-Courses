...
@Service
public class AnyCourseListPublishedService implements AbstractListService<Any, Course> {
	...
	public Collection<Course> findMany(final Request<Course> request) {
		...
		result = this.repository.findManyPublishedCourses(true);
	...
	public void unbind(final Request<Course> request, final Course entity, final Model model) {
		...
		request.unbind(entity, model, "ticker", "caption", "link");
