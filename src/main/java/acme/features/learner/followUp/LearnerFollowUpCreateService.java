...
public class LearnerFollowUpCreateService implements AbstractCreateService<Learner, FollowUp> {
	...
	public void validate(final Request<FollowUp> request, final FollowUp entity, final Errors errors) {
		...
		errors.state(request, confirmation, "confirmation", "learner.follow-up.confirmation.error");
		...
		if (!errors.hasErrors("sequenceNumber")) {
			...
			errors.state(request, f == null || f.getId() == entity.getId(), "sequenceNumber", "learner.follow-up.form.error.duplicated");
		}
	}
